...
Deploy DHCP services so Windows clients can receive consistent IP addressing, gateway, DNS, and lease settings automatically.
...
| Step | Description |
|------|-------------|
| Install the DHCP Server role | The DHCP Server role is selected in Server Manager. |
| Authorize DHCP in Active Directory | The DHCP server is authorized so it can issue leases in the AD domain environment. |
| Create the DHCP scope | The DHCP scope defines the client address range. |
| Add exclusions | The first addresses in the range are excluded so they can remain reserved for static infrastructure or other planned use. |
| Configure DHCP options | The scope provides gateway and DNS settings to clients. |
| Validate automatic client configuration | The Windows 10 client receives its IP configuration automatically from DHCP. |
| Configure DHCP failover | The lab configures a DHCP failover relationship with DC2. |
